...
// Note: Teams format matches are NOT weighted (courts don't represent difficulty in teams mode)
const GROUP_WEIGHTS = {
  1: 1.5,
  2: 1.25,
  default: 1.0
  ...
  const result = await query(`
    SELECT 
      ...
      COUNT(DISTINCT CASE 
        ...
        ELSE NULL 
      END) as losses,
      -- Wins by group for weighted calculation (only 'group' format gets group weighting)
      COUNT(DISTINCT CASE 
        WHEN m.match_group = 1 AND g.format = 'group' AND (
          (m.winner = 'teamA' AND (m.team_a_player1 = a.id OR m.team_a_player2 = a.id))
          OR (m.winner = 'teamB' AND (m.team_b_player1 = a.id OR m.team_b_player2 = a.id))
        ) THEN m.id ELSE NULL 
      END) as group1_wins,
      COUNT(DISTINCT CASE 
        WHEN m.match_group = 2 AND g.format = 'group' AND (
          (m.winner = 'teamA' AND (m.team_a_player1 = a.id OR m.team_a_player2 = a.id))
          OR (m.winner = 'teamB' AND (m.team_b_player1 = a.id OR m.team_b_player2 = a.id))
        ) THEN m.id ELSE NULL 
      END) as group2_wins,
